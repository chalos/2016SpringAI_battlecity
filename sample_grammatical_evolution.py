...
<expr>              ::= <expr> <biop> <expr> | <uop> <expr> | <real> |
                        ...
                        self.runtime_resolve('<value>', 'float')
<biop>              ::= + | - | * | /
<uop>               ::= + | -
<pow>               ::= pow(<expr>, <real>)
<plus>              ::= +
...
for i in xrange(100):
    ...
    total += abs( <expr> - (pow(value, 3)*value))
fitness = total
print self
...
ges.set_max_generations(2000)
ges.set_fitness_type('min', 0.01)

ges.set_max_program_length(4000)

ges.set_wrap(True)
ges.set_fitness_fail(9999999)
